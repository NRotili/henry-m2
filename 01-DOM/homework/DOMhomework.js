...
// Tu código acá:
document.querySelector('#createdBy').innerHTML += ' friss';
...
ToDo.prototype.completeToDo = function () { this.complete = !this.complete; } // Para volver a dejar pendiente la tarea
...
function buildToDo(todo, index) {
  ...
  toDoRemove.className = 'fas fa-trash-alt';
  toDoRemove.id = 'r' + index;
  toDoRemove.addEventListener('click', removeTask);

  const toDoCheck = document.createElement('input');
  toDoCheck.type = 'checkbox';  // toDoCheck.setAttribute('type', 'checkbox');
  toDoCheck.id = index;
  ...
  if (todo.complete) {
    ...
    toDoShell.classList.remove('completeText');
    toDoCheck.checked = false;
  }

  toDoShell.appendChild(toDoCheck);
  toDoShell.appendChild(toDoText);
  toDoShell.appendChild(toDoRemove);
...
function displayToDos() {
  // Tu código acá:
  const toDoContainer = document.getElementById('toDoContainer');   // const toDoContainer = document.querySelector('#toDoContainer');
...
function addToDo() {
  // Tu código acá:
  const toDoInput = document.getElementById('toDoInput');   // const toDoInput = document.querySelector('#toDoInput');
...
// Tu código acá:
const addButton = document.getElementById('addButton');   // const addButton = document.querySelector('#addButton');
addButton.addEventListener('click', addToDo); // addButton.onclick = addToDo;
...
function completeToDo(event) {
  ...
  // Tu código acá:
  console.dir('checked vale ' + event.target.checked);
...
function removeTask(event) {
  ...
  console.log('index para remover: ' + index);
